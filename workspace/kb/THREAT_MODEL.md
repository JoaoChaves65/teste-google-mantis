...
BarberLab is a monorepo-based barbershop management system with **dual-API architecture**:

- **`@barberlab/api-secure`** (port 3001): Production-ready API with full security controls
- **`@barberlab/api-vulnerable`** (port 3002): Intentionally vulnerable API for security training/labs
- **`@barberlab/core`**: Shared domain, application logic, persistence interfaces, and infrastructure
- **`@barberlab/web`** (port 5173): React + Vite frontend consuming the secure API

Both APIs share the **same core domain logic** (`@barberlab/core`) but implement **different security postures** at the HTTP/presentation layer.
...
This system is an educational Security Lab. The vulnerable API variant (`api-vulnerable`) is intentionally designed with security weaknesses for training purposes. The secure variant (`api-secure`) demonstrates proper security controls.
...
5. ✅ NO entity documents a real untrusted external input crossing into privileged logic
...
**Vector:** `VulnerableAppointmentRepository.findByCustomerId('1\' OR \'1\'=\'1 --')`  
...
**Vector:** `GET /api/v1/customers/{other_customer_id}` as authenticated Customer  
...
This is a Security Lab for educational purposes. The vulnerable variant exists solely for controlled security training. The secure variant demonstrates proper mitigations.
...
*Threat Model generated by Mantis Threat Modeler — KB_SNAPSHOT: snap_20250829_01*