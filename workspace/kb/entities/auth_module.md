...
## Overview
Authentication and authorization middleware for both API variants. Handles JWT validation, token refresh, and role-based access control.
...
## Trajectory Insights
- **ETAPA 12**: 3 pre-existing test failures in api-secure (SameSite, rate limiting) unrelated to vulnerabilities
- **ETAPA 10**: api-vulnerable auth lacks revocation check → token theft impact amplified
- **ETAPA 12**: Rate limiting test expects 429 but gets 400/401 → implementation gap
